...
スマホ縦画面向けのWebアプリとして作成しています。現在は `public/monsters/<コード>-<企業名>.png` の画像ファイルから企業モンスターを自動生成し、ガチャ、放置報酬、図鑑、チーム、マーケット購入、株数管理を動かします。
...
画像追加後は次のコマンドで `lib/companyMonsterAssets.ts` を更新します。

```bash
npm run sync:monsters
```
